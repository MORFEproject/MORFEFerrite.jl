...
function comsol_to_gmsh(comsol_file::String, gmsh_file::String)
    ...
    if length(e2nH27) > 0
        elemType = 12
        elemTags = e2gH27
        # perm_H27[j] = COMSOL position corresponding to Gmsh type-12 position j.
        # Derived by matching the COMSOL hex2 coordinates against the reference
        # coordinates returned by gmsh.model.mesh.getElementProperties(12).
