...
# Backends pass the `meta` NamedTuple returned by `build_model`. The `.info`
# branch remains useful for assembled cases, whose construction timings live
# alongside their matrices and mesh data.
...
[`stage_timings`](@ref) from both `case.info` and the ROM `meta`, then whatever
