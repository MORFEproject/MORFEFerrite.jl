...
# Gate A (small): the public workflow agrees with an explicit reference solve.
...
@testset "high-level vs hand-built (Gate A, small)" begin
    ...
    rom = svk_solve(beam; master = [1], order = _order)
    @test rom.meta.backend == "Ferrite"
    @test rom.meta.master_indices == [1, 2]

    # Explicit reference solve on an identical grid.
    grid2 = _test_grid()
...
@testset "zero-amplitude forcing consistency (Gate B, small)" begin
    ...
    rom0 = svk_solve(beam; master = [1], order = _order)
    romf = svk_solve(beam; master = [1], order = _order,
        forcing = SVK.HarmonicForcing(mode = 1, amplitude = 0.0))

    @test romf.meta.N_EXT == 2
    @test length(romf.meta.forcings) == 1
    @test romf.meta.Ω[1] ≈ abs(rom0.meta.spectrum.eigenvalues[1])
...
@testset "multiple forcings" begin
    ...
    romsel = svk_solve(beam; master = [1], order = _order,
        forcing = [SVK.HarmonicForcing(mode = 1, amplitude = 0.1),
            SVK.HarmonicForcing(mode = 1, amplitude = 0.0, Ω = 3.0 * ω1)])
...
@testset "outer-resonance diagnostic" begin
    ...
    square_beam = SVK.mechanical_model(sq;
        ...
        dirichlet = "Dirichlet", fe_order = 2, quad_order = 3)
    warns0, rom0 = _run() do
        svk_solve(square_beam; master = [1], order = 3)
    end
    @test isempty(rom0.meta.forcings)              # genuinely autonomous
    ...
    ω2 = abs(λ[3])
    warns3, _ = _run() do
        svk_solve(_beam(0.0, 0.6 / ω2^2); master = [1], order = 3,
            forcing = SVK.HarmonicForcing(mode = 2, amplitude = 0.1))
    end
    @test isempty(warns3)
    ...
    warns4, _ = _run() do
        svk_solve(undamped; master = [1, 2], order = 3,
            forcing = SVK.HarmonicForcing(mode = 2, amplitude = 0.1))
    end
    @test isempty(warns4)
end
...
@testset "error paths" begin
    ...
    @test_throws AssertionError svk_solve(beam; master = [2, 1], order = 3)
    @test_throws AssertionError svk_solve(beam; master = [1], order = 3,
