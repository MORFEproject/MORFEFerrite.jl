...
function iso_crystal(; rotation = nothing)
    SVK.CubicCrystal(
...
@testset "assembled operators: anisotropic ≡ isotropic in the limit" begin
    ...
    Ki = allocate_matrix(dh)
    Mi = allocate_matrix(dh)
    Ka = allocate_matrix(dh)
    Ma = allocate_matrix(dh)
    ...
    for deg in (2, 3)
        ...
        ri = zeros(ComplexF64, n)
        ra = zeros(ComplexF64, n)
...
@testset "ROM: anisotropic path reproduces the isotropic ROM" begin
    ...
    ri = svk_solve(miso; master = [1], order = 3)
    ra = svk_solve(mani; master = [1], order = 3)
